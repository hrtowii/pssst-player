...
void draw_glyph(int x, int y, unsigned char c, RGBA8888 color) {
  ...
  v[0].y = y;
  v[0].z = 0;
  // this is some bullshit math that i wish i could understand like i watchd a
  // 20 min video and gave up
...
void draw_text8x16(int x, int y, const char *str, int len, RGBA8888 color) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    v[vi].u = (float)(cell_x + GLYPH_W);
    v[vi].v = (float)(cell_y + GLYPH_H);
    v[vi].color = gu_color;
    v[vi].x = cursor_x + GLYPH_W;
    v[vi].y = y + GLYPH_H;
    v[vi].z = 0;
    vi++;
