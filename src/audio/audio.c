...
bool audio_init(void) {
    ...
    g_audio_channel = sceAudioChReserve(PSP_AUDIO_NEXT_CHANNEL,
                                         AUDIO_CHUNK_FRAMES, PSP_AUDIO_FORMAT_STEREO);
...
bool audio_playlist_set(const char **paths, int count) {
    if (count > AUDIO_MAX_TRACKS) return false;
    sceKernelWaitSema(g_pl.lock, 1, NULL);
    for (int i = 0; i < g_pl.count; i++) free(g_pl.paths[i]);
    for (int i = 0; i < count; i++) g_pl.paths[i] = strdup(paths[i]);
    g_pl.count = count;
    g_pl.current = -1;
...
bool audio_playlist_add(const char *path) {
    sceKernelWaitSema(g_pl.lock, 1, NULL);
    if (g_pl.count >= AUDIO_MAX_TRACKS) { sceKernelSignalSema(g_pl.lock, 1); return false; }
...
void audio_play_index(int index) {
    sceKernelWaitSema(g_pl.lock, 1, NULL);
    if (index < 0 || index >= g_pl.count) { sceKernelSignalSema(g_pl.lock, 1); return; }
    g_pl.current = index;
...
void audio_play(void) {
    if (g_state == AUDIO_STATE_PAUSED) g_state = AUDIO_STATE_PLAYING;
}

void audio_pause(void) {
    if (g_state == AUDIO_STATE_PLAYING) g_state = AUDIO_STATE_PAUSED;
}
...
void audio_next(void) {
    ...
    if (path) request_switch_to(path);
    else request_stop();
}

void audio_prev(void) {
    sceKernelWaitSema(g_pl.lock, 1, NULL);
    if (g_pl.count == 0) { sceKernelSignalSema(g_pl.lock, 1); return; }
    int prev = g_pl.current - 1;
    if (prev < 0) prev = (g_pl.repeat == REPEAT_ALL) ? g_pl.count - 1 : 0;
    g_pl.current = prev;
